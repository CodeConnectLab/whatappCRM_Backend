/**
 * Full demo dataset: user login, company, wallet, Twilio placeholders, contacts,
 * groups, templates, campaigns, chats/messages, sample media & activity.
 *
 * From backend/:  npm run seed
 *
 * Login after seed (defaults):
 *   demo@demo.local / DemoPass123!
 *
 * Env overrides:
 *   SEED_DEMO_EMAIL, SEED_DEMO_PASSWORD, SEED_COMPANY_NAME
 *   SEED_APPEND_MESSAGES=1 — add another message batch to demo chats (same threads)
 *   SEED_FORCE_CHAT_SEED=1 — delete seeded-thread messages for this company and re-insert (use to gain 12 chats’ new history after an old seed)
 *   Re-run seed to upsert templates; without force, chats that already have messages are skipped.
 */
...
const EXTRA_CONTACTS: { phone: string; name: string; tags: string[] }[] = [
...
/** 12 inbox rows for scroll testing (8 workspace contacts + 4 extra). */
const CHAT_SEED_CONTACTS: { phone: string; name: string }[] = [
...
async function ensureDemoPrincipal(): Promise<{ companyId: Types.ObjectId; userId: Types.ObjectId }> {
...
async function ensureTwilioAndNumber(companyId: Types.ObjectId) {
...
async function upsertTemplates(companyId: Types.ObjectId): Promise<Types.ObjectId[]> {
  const defs = [
...
async function seedContacts(companyId: Types.ObjectId): Promise<Types.ObjectId[]> {
  const ids: Types.ObjectId[] = [];
...
async function seedGroups(companyId: Types.ObjectId, contactIds: Types.ObjectId[]) {
...
async function seedChats(
  ...
  contactSubset: { phone: string; name: string }[],
) {
  const append = process.env.SEED_APPEND_MESSAGES === '1' || process.env.SEED_APPEND_MESSAGES === 'true';
  const forceReseed =
    process.env.SEED_FORCE_CHAT_SEED === '1' || process.env.SEED_FORCE_CHAT_SEED === 'true';
  for (const contact of contactSubset) {
    ...
    const baseOffset = append ? countAfter * 120_000 : 0;
    const now = Date.now();
    const docs = templates.map((t, i) => {
      const createdAt = new Date(now - (templates.length - i) * 60_000 - baseOffset);
      const outboundStatus = t.direction === 'outbound' ? t.status ?? 'delivered' : undefined;
      return {
        companyId,
        chatId: chat._id,
        ...
        ...(t.direction === 'outbound'
          ? { status: outboundStatus, senderUserId }
          : { status: 'read' }),
        createdAt,
        updatedAt: createdAt,
      };
    ...
    await ChatModel.updateOne(
      ...
      {
        $set: {
          lastMessageAt: docs[docs.length - 1]?.createdAt ?? new Date(),
...
async function seedMediaAndActivity(companyId: Types.ObjectId, userId: Types.ObjectId) {
...
async function ensureSampleTransaction(companyId: Types.ObjectId) {
