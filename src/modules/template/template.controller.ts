...
import { TemplateModel, type TemplateStatus, type TemplateCategory } from './template.model.js';
import {
  extractVariables,
  toPositionalBody,
  toMetaTemplateName,
...
export async function createTemplate(req: Request, res: Response): Promise<void> {
  const companyId = req.companyId!;
  const { name, body, language, imageUrl, category } = req.body as {
    name: string;
    body: string;
    language?: string;
    imageUrl?: string;
...
export async function updateTemplate(req: Request, res: Response): Promise<void> {
  ...
  if (body.category !== undefined) setDoc.category = body.category;
...
export async function syncTemplates(req: Request, res: Response): Promise<void> {
  ...
  for (const t of locals) {
    ...
    t.set({
      ...
      status: match.status as TemplateStatus,
      ...(match.category ? { category: match.category as TemplateCategory } : {}),
