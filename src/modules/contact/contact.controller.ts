...
export function downloadContactImportTemplate(_req: Request, res: Response): void {
...
export async function createContact(req: Request, res: Response): Promise<void> {
  const companyId = req.companyId!;
  const { phone, name, email, tags } = req.body as {
    phone: string;
    name?: string;
    email?: string;
    tags?: string[];
...
export async function updateContact(req: Request, res: Response): Promise<void> {
  const companyId = req.companyId!;
  const { id } = req.params;
  const body = req.body as Partial<{ name: string; email: string; tags: string[]; phone: string }>;
  const c = await ContactModel.findOneAndUpdate(
...
export async function importContactsCsv(req: Request, res: Response): Promise<void> {
  const companyId = req.companyId!;
  const file = req.file;
  const bodyGroup =
  ...
  const phoneColumn =
    phoneIdx >= 0 ? phoneIdx : mobileIdx >= 0 ? mobileIdx : phoneNumberIdx >= 0 ? phoneNumberIdx : -1;
  ...
  const importedIds: Types.ObjectId[] = [];
  const seenIdHex = new Set<string>();
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      const cid = doc._id as Types.ObjectId;
  ...
  if (bodyGroup && importedIds.length > 0) {
    await ContactGroupModel.updateOne(
      { companyId: oid, name: bodyGroup, deletedAt: null },
      {
...
export async function updateGroup(req: Request, res: Response): Promise<void> {
  ...
  const { id } = req.params;
  const { name, contactIds } = req.body as { name?: string; contactIds?: string[] };
  const g = await ContactGroupModel.findOneAndUpdate(
    { _id: id, companyId: new Types.ObjectId(companyId), deletedAt: null },
    {
      $set: {
        ...(name ? { name } : {}),
