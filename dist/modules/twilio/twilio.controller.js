...
async function upsertTwilioAccount(req, res) {
...
async function listTwilioAccounts(req, res) {
  const companyId = req.companyId;
...
async function upsertWhatsappNumber(req, res) {
  const companyId = req.companyId;
  const body = req.body;
  ...
  const existingMeta = provider === "meta" ? await WhatsappNumberModel.findOne({
    companyId: companyOid,
    provider: "meta",
    deletedAt: null
  }).sort({ isDefault: -1, updatedAt: -1 }).lean() : null;
  ...
  try {
    if (provider === "meta") {
      const doc2 = await WhatsappNumberModel.findOneAndUpdate(
