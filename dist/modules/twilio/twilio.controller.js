...
async function releasePhoneFromTombstones(companyOid, phoneNumber) {
    ...
    for (const row of tombstones) {
        await WhatsappNumberModel.updateOne({ _id: row._id }, { $set: { phoneNumber: `${phoneNumber}#archived-${String(row._id)}` } });
    }
}
export async function upsertTwilioAccount(req, res) {
    const companyId = req.companyId;
    const { accountSid, authToken, friendlyName } = req.body;
    const authTokenEncrypted = encryptSecret(authToken);
    const doc = await TwilioAccountModel.findOneAndUpdate({ companyId: new Types.ObjectId(companyId), accountSid }, {
...
export async function listTwilioAccounts(req, res) {
    const companyId = req.companyId;
...
export async function upsertWhatsappNumber(req, res) {
    const companyId = req.companyId;
    const body = req.body;
    ...
    const existingMeta = provider === 'meta'
        ? await WhatsappNumberModel.findOne({
            ...
            .lean()
        : null;
    const isDefault = body.isDefault ?? (provider === 'meta' ? (existingMeta?.isDefault ?? true) : false);
    if (isDefault) {
        await WhatsappNumberModel.updateMany({ companyId: companyOid, provider }, { $set: { isDefault: false } });
    }
    const setDoc = {
        provider,
    ...
    try {
        if (provider === 'meta') {
            const doc = await WhatsappNumberModel.findOneAndUpdate({ companyId: companyOid, provider: 'meta', deletedAt: null }, {
                $set: setDoc,
                $unset: unsetDoc,
            ...
            for (const dup of duplicates) {
                await WhatsappNumberModel.updateOne({ _id: dup._id }, {
                    $set: {
                        deletedAt: now,
                        isDefault: false,
        ...
        const doc = await WhatsappNumberModel.findOneAndUpdate({ companyId: companyOid, phoneNumber: body.phoneNumber }, {
            $set: twilioFields,
            $unset: unsetDoc,
...
export async function listWhatsappNumbers(req, res) {
    const companyId = req.companyId;
