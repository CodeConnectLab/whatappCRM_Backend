...
whatsappNumberSchema.index({ companyId: 1, phoneNumber: 1 }, { unique: true, partialFilterExpression: { deletedAt: null } });
whatsappNumberSchema.index({ companyId: 1, provider: 1 }, {
    unique: true,
    partialFilterExpression: { provider: 'meta', deletedAt: null },
});
whatsappNumberSchema.index({ metaPhoneNumberId: 1 }, { unique: true, sparse: true, partialFilterExpression: { metaPhoneNumberId: { $type: 'string' } } });
export const WhatsappNumberModel = getModel('WhatsappNumber', whatsappNumberSchema);
