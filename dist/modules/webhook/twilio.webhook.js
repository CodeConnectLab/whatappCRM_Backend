...
async function twilioIncomingController(req, res) {
  const signatureValid = validateTwilio(req);
  const companyIdHeader = req.header("x-company-id");
  try {
    const payload = req.body;
    await WebhookLogModel.create({
      companyId: companyIdHeader && Types.ObjectId.isValid(companyIdHeader) ? new Types.ObjectId(companyIdHeader) : void 0,
      path: req.path,
      payload,
      headers: req.headers,
      signatureValid
    ...
    await ChatModel.updateOne(
      { _id: chatId },
      {
        $set: { lastMessageAt: /* @__PURE__ */ new Date(), lastMessagePreview: body.slice(0, 140) },
        $inc: { unreadCount: 1 }
      }
    );
...
async function twilioStatusController(req, res) {
  const signatureValid = validateTwilio(req);
  try {
    const payload = req.body;
    await WebhookLogModel.create({
