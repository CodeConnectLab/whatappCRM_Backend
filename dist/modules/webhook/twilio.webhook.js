...
function validateTwilio(req) {
    if (!env.TWILIO_WEBHOOK_AUTH_TOKEN)
        return true;
    ...
    return twilio.validateRequest(env.TWILIO_WEBHOOK_AUTH_TOKEN, signature, url, req.body);
}
export async function twilioIncomingController(req, res) {
    const signatureValid = validateTwilio(req);
    const companyIdHeader = req.header('x-company-id');
    try {
        const payload = req.body;
        await WebhookLogModel.create({
            companyId: companyIdHeader && Types.ObjectId.isValid(companyIdHeader)
...
export async function twilioStatusController(req, res) {
    const signatureValid = validateTwilio(req);
    try {
        const payload = req.body;
        await WebhookLogModel.create({
