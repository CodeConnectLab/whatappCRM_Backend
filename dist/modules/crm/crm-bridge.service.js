...
export async function getCrmBridge(companyId) {
...
export async function upsertCrmBridge(companyId, input) {
    ...
    if (input.crmBaseUrl !== undefined)
        set.crmBaseUrl = input.crmBaseUrl.replace(/\/$/, '');
    if (input.crmApiKey)
        set.crmApiKeyEncrypted = encryptSecret(input.crmApiKey);
    if (input.pushMode !== undefined)
        set.pushMode = input.pushMode;
    if (input.leadSourceLabel !== undefined)
        set.leadSourceLabel = input.leadSourceLabel || undefined;
    ...
    const doc = await CrmBridgeModel.findOneAndUpdate({ companyId: oid }, { $set: set, $setOnInsert: { companyId: oid } }, { upsert: true, new: true }).lean();
    return toView(doc);
}
export async function deleteCrmBridge(companyId) {
    await CrmBridgeModel.updateOne({ companyId: new Types.ObjectId(companyId) }, { $set: { deletedAt: new Date(), enabled: false } });
}
 ...
 */
export async function pushChatToCrm(companyId, chatId) {
    const oid = new Types.ObjectId(companyId);
    const bridge = await CrmBridgeModel.findOne({ companyId: oid, deletedAt: null }).lean();
    if (!bridge?.enabled || !bridge.crmBaseUrl || !bridge.crmApiKeyEncrypted)
        return 'skipped';
    ...
    if (chat.crmSyncStatus === 'synced' || chat.crmSyncStatus === 'duplicate')
        return 'skipped';
    ...
    try {
        ...
        await ChatModel.updateOne({ _id: chat._id }, { $set: { crmSyncStatus: 'failed', crmSyncError: 'API key could not be decrypted' } });
        return 'failed';
    }
    ...
    try {
        const res = await axios.post(`${bridge.crmBaseUrl}/outsource-lead`, payload, {
        ...
        const outcome = duplicate ? 'duplicate' : 'synced';
        ...
        await CrmBridgeModel.updateOne({ companyId: oid }, {
 ...
 */
export async function testCrmBridge(companyId) {
    const oid = new Types.ObjectId(companyId);
    const bridge = await CrmBridgeModel.findOne({ companyId: oid, deletedAt: null }).lean();
    ...
    try {
        const res = await axios.post(`${bridge.crmBaseUrl}/outsource-lead`, {
        ...
        const duplicate = Boolean(res.data?.data?.duplicate);
        await CrmBridgeModel.updateOne({ companyId: oid }, { $set: { lastPushAt: new Date(), lastPushStatus: 'test-ok' }, $unset: { lastPushError: '' } });
        return {
            ok: true,
            message: duplicate
        ...
        const message = errorText(e);
        await CrmBridgeModel.updateOne({ companyId: oid }, { $set: { lastPushAt: new Date(), lastPushStatus: 'test-failed', lastPushError: message } });
