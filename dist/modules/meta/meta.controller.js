...
import { buildMetaWebhookUrl, generateWebhookSlug, generateWebhookVerifyToken, } from './meta-webhook.utils.js';
...
export async function getMetaWhatsappConfig(req, res) {
    ...
    if (doc && (!doc.webhookSlug || !doc.webhookVerifyToken)) {
        const patch = {};
        if (!doc.webhookSlug)
            patch.webhookSlug = await ensureUniqueWebhookSlug();
        if (!doc.webhookVerifyToken)
            patch.webhookVerifyToken = generateWebhookVerifyToken();
        doc = await MetaWhatsappConfigModel.findOneAndUpdate({ companyId: oid }, { $set: patch }, { new: true }).lean();
    }
    if (!doc) {
        res.json({
...
export async function upsertMetaWhatsappConfig(req, res) {
    const companyId = req.companyId;
    const body = req.body;
    const oid = new Types.ObjectId(companyId);
    ...
    const doc = await MetaWhatsappConfigModel.findOneAndUpdate({ companyId: oid }, update, { upsert: true, new: true }).lean();
    await logActivity({
        companyId,
