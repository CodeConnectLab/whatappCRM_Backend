...
export async function getMetaReadiness(companyId) {
    ...
    const defaultSender = senders.find((s) => s.isDefault && s.metaPhoneNumberId) ??
        senders.find((s) => s.metaPhoneNumberId);
    const issues = [];
    if (!credentialsConfigured)
        issues.push('Meta access token and app secret are not saved');
    if (!senderConfigured)
        issues.push('No Meta WhatsApp sender (phone number ID) configured');
    if (!webhookVerified)
        issues.push('Meta webhook is not verified yet');
    return {
        credentialsConfigured,
        senderConfigured,
        webhookVerified,
        defaultSenderId: defaultSender ? String(defaultSender._id) : undefined,
        readyForOutbound: credentialsConfigured && senderConfigured,
        issues,
    };
}
export async function assertCampaignCanStart(companyId, input) {
    ...
    // Campaigns land outside the 24-hour window, so Meta only delivers approved templates.
    if (wa.provider === 'meta') {
        ...
        if (tpl.status !== 'APPROVED') {
            ...
            throw new Error(`Template "${tpl.name}" is ${state} — submit it for Meta approval and wait until it is approved before starting a campaign`);
        }
    }
