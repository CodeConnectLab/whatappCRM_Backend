...
async function creditCompanyWallet(req, res) {
...
async function listTeam(req, res) {
  ...
  const members = await MembershipModel.find({ companyId: new Types.ObjectId(companyId), deletedAt: null }).populate("userId", "email name").lean();
  res.json(members);
}
