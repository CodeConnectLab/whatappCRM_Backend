...
async function addCampaignJob(companyId, campaignId, contactId) {
  try {
    await campaignQueue.add(
      "send",
      { companyId, campaignId, contactId },
      {
...
async function enqueueCampaign(campaignId, companyId, userId) {
  ...
  const contactIdSet = /* @__PURE__ */ new Set();
  ...
  if (campaign.templateId) {
    ...
    mediaUrl = rawUrl.length > 0 ? rawUrl : void 0;
  }
  if (!body) throw new Error("Campaign has no template body");
  ...
  await CampaignModel.updateOne(
    { _id: campaign._id },
    { $set: { status: "running", stats: { total: contacts.length, sent: 0, failed: 0 } } }
  );
...
async function pauseCampaign(campaignId, companyId) {
  await CampaignModel.updateOne(
    { _id: new Types.ObjectId(campaignId), companyId: new Types.ObjectId(companyId) },
    { $set: { status: "paused" } }
  );
}
async function resumeCampaign(campaignId, companyId, userId) {
  await CampaignModel.updateOne(
    { _id: new Types.ObjectId(campaignId), companyId: new Types.ObjectId(companyId) },
    { $set: { status: "running" } }
  );
