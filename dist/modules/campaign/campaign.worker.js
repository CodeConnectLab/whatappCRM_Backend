...
export function startCampaignWorker() {
    return new Worker(CAMPAIGN_QUEUE, async (job) => {
        ...
        if (!cm || cm.status === 'sent' || cm.twilioSid)
            return;
        await CampaignMessageModel.updateOne({ _id: cm._id, status: { $ne: 'sent' } }, { $set: { status: 'queued' } });
        const contact = await ContactModel.findById(contactId).lean();
        if (!contact) {
            await CampaignMessageModel.updateOne({ _id: cm._id }, { $set: { status: 'failed', error: 'contact missing' } });
            return;
        }
        ...
        if (!wa) {
            await CampaignMessageModel.updateOne({ _id: cm._id }, { $set: { status: 'failed', error: 'whatsapp number missing' } });
            return;
        }
        ...
        if ((wa.provider ?? 'twilio') === 'meta' && !useTemplate) {
            ...
            await CampaignMessageModel.updateOne({ _id: cm._id }, { $set: { status: 'failed', error: reason } });
        ...
        try {
            const { sid } = useTemplate
                ? await sendWhatsappTemplateMessage({
                    ...
                    parameters: buildParameterValues((tpl.variables ?? []), contact),
