...
async function createCampaign(req, res) {
  ...
  const c = await CampaignModel.create({
    companyId: new Types.ObjectId(companyId),
    name: body.name,
    templateId: body.templateId ? new Types.ObjectId(body.templateId) : void 0,
    ...
    scheduledAt: body.scheduledAt ? new Date(body.scheduledAt) : void 0,
    status: "draft"
  });
...
async function startCampaignCtrl(req, res) {
  const companyId = req.companyId;
  const { id } = req.params;
  if (!id) {
...
async function pauseCampaignCtrl(req, res) {
  const companyId = req.companyId;
  const { id } = req.params;
  if (!id) {
    res.status(400).json({ error: "Missing campaign id" });
...
async function resumeCampaignCtrl(req, res) {
  const companyId = req.companyId;
  const { id } = req.params;
  if (!id) {
    res.status(400).json({ error: "Missing campaign id" });
...
async function deleteCampaign(req, res) {
  ...
  await CampaignModel.updateOne(
    ...
    { $set: { deletedAt: /* @__PURE__ */ new Date(), status: "paused" } }
  );
  res.json({ ok: true });
}
export {
