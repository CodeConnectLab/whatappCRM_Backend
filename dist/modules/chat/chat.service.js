...
async function listChats(companyId) {
  return ChatModel.find({ companyId: new Types.ObjectId(companyId), deletedAt: null }).sort({ lastMessageAt: -1 }).populate("contactId", "name phone").lean();
}
async function listMessages(companyId, chatId, opts) {
...
async function openChatWithContact(input) {
  ...
  const populated = await ChatModel.findById(chat._id).populate("contactId", "name phone").lean();
  if (!populated) throw new Error("Chat not found");
  return populated;
}
async function ensureInboundChat(input) {
