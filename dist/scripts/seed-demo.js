...
async function ensureDemoPrincipal() {
    ...
    if (!m?.companyId)
        throw new Error('Demo user has no company membership');
...
async function upsertTemplates(companyId) {
    ...
    const ids = [];
    for (const t of defs) {
        const doc = await TemplateModel.findOneAndUpdate({ companyId, name: t.name, deletedAt: null }, {
            $set: {
                body: t.body,
                language: t.language,
        ...
        if (doc?._id)
            ids.push(new Types.ObjectId(String(doc._id)));
    }
    return ids;
}
async function seedContacts(companyId) {
    ...
    for (const c of EXTRA_CONTACTS) {
        const doc = await ContactModel.findOneAndUpdate({ companyId, phone: c.phone }, { $set: { name: c.name, tags: [...c.tags], deletedAt: null }, $setOnInsert: { companyId, phone: c.phone } }, { upsert: true, new: true }).lean();
        if (doc?._id)
            ids.push(new Types.ObjectId(String(doc._id)));
    }
    return ids;
...
async function seedGroups(companyId, contactIds) {
    ...
    await ContactGroupModel.findOneAndUpdate({ companyId, name: 'VIP customers' }, { $set: { contactIds: vip.length ? vip : contactIds.slice(0, 2), deletedAt: null }, $setOnInsert: { companyId, name: 'VIP customers' } }, { upsert: true });
    await ContactGroupModel.findOneAndUpdate({ companyId, name: 'Newsletter' }, {
...
async function seedChats(companyId, whatsappNumberId, senderUserId, contactSubset) {
    ...
    for (const contact of contactSubset) {
        ...
        if (countAfter > 0 && !append && !forceReseed)
            continue;
...
async function main() {
    ...
    if (mainTemplate)
        await seedCampaigns(companyId, mainTemplate, whatsappNumberId, contactIds);
